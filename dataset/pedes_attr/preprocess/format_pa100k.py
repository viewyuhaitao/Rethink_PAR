...
def generate_data_description(save_dir, reorder):
    ...
    dataset.partition.train = np.arange(0, 1600)  # np.array(range(80000))
    dataset.partition.val = np.arange(1600, 1800)  # np.array(range(80000, 90000))
    dataset.partition.test = np.arange(1800, 2000)  # np.array(range(90000, 100000))
    dataset.partition.trainval = np.arange(0, 1800)  # np.array(range(90000))
...
if __name__ == "__main__":
    ...
    save_dir = 'D:\penson_attributes\Rethinking_of_PAR\data\PA100k'
    reoder = True
