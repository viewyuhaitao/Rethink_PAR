...
from models.backbone.resnet import resnet50  # 确保 resnet50 被导入
...
def check_registry():
    try:
        # 检查 BACKBONE 注册表是否已经初始化
        assert isinstance(BACKBONE, Registry), "BACKBONE registry is not initialized correctly"

        # 检查 resnet50 模型是否已经注册
        resnet50_fn = BACKBONE.get("resnet50")
        assert callable(resnet50_fn), "resnet50 model is not registered correctly"

        print("BACKBONE registry is initialized correctly and resnet50 model is registered.")
