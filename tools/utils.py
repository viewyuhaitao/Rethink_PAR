...
class ReDirectSTD(object):
    ...
    #                 # self.f = open(r'{}'.format(self.file), 'w')
    #                 # 修改后的代码
    #                 # self.f = open(os.path.join(*self.file.split('/')), 'w')
    ...
    def write(self, msg):
        ...
        if self.file is not None:
            # 替换文件名中的冒号为下划线
            safe_filename = os.path.basename(self.file).replace(':', '_')
            safe_filepath = os.path.join(os.path.dirname(self.file), safe_filename)

            # 检查目录是否存在，不存在则创建
            if not os.path.exists(os.path.dirname(safe_filepath)):
                os.makedirs(os.path.dirname(safe_filepath))

            if self.immediately_visiable:
                with open(safe_filepath, 'a') as f:
                    f.write(msg)
            else:
                if self.f is None:
                    self.f = open(safe_filepath, 'w')
                self.f.write(msg)
